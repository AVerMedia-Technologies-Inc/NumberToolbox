...
function save(){
  var json = {
    ...
    context: window.PLUGIN_UUID,
    payload: {
      "min": 5
    }
  }
  AVT_CREATOR_CENTRAL.connection && AVT_CREATOR_CENTRAL.connection.sendJSON(json);
}
...
AVT_CREATOR_CENTRAL = (function() {
  ...
  function init() {
    ...
    function connect(port, UID, inRegisterEvent, inInfo, inActionInfo) {
      inPort = port;
      inUUID = UID;
      inMessageType = inRegisterEvent; //"ax.register.widget"; or "ax.register.property"
