...
const AVT_CREATOR_CENTRAL_API = {
    ...
    sendToSound : function (payload) {
        AVT_CREATOR_CENTRAL_API.send('ax.get.sound.output.list', payload);
    },
    getToSound : function (payload) {
        AVT_CREATOR_CENTRAL_API.send('ax.update.sound.output.list', payload);
    },
    sendToFile : function (payload) {
        AVT_CREATOR_CENTRAL_API.send('ax.select.audio.file', payload);
        
    },
    getToFile : function (payload) {
        AVT_CREATOR_CENTRAL_API.send('ax.update.select.file.name', payload);
    },
    sendToReview : function (payload) {
        AVT_CREATOR_CENTRAL_API.send('ax.review.fileinfo', {
            'filePaths': payload
        });
    },
    getToReview : function (payload) {
        AVT_CREATOR_CENTRAL_API.send('ax.update.existing.fileinfo', payload);
    },
    sendToControl: function (state,filename,devices) {
        AVT_CREATOR_CENTRAL_API.send('ax.control.sound', {
            'controlType': Number(state),
            'fileName': filename,
...
AVT_CREATOR_CENTRAL = (function() {
    ...
    function init() {
        ...
        function connect(port, UID, inRegisterEvent, inInfo, inActionInfo) {
            inPort = port;
            inUUID = UID;
            inMessageType = "ax.register.widget";
...
// connectCreatorCentral(53921,'cOb57e75-55c5-4595-ad7d-e3d2293d9deO','ax.register.widget')